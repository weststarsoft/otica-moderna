...
export default function OculosDeSol() {
    ...
    <div className="min-h-screen bg-[#f5f5f5] text-black font-sans">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link, i) => (
              <a
                key={link}
                href="#"
                className={`text-xs font-bold tracking-widest hover:text-[#C41A1A] transition-colors ${i === 1 ? 'text-[#C41A1A]' : 'text-gray-800'}`}
                onClick={(e) => {
                  e.preventDefault()
                  if (i === 0) router.push('/')
                  if (i === 1) router.push('/oculos-de-sol')
                }}
              >
                {link}
              </a>
      ...
      {menuAberto && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-95 flex flex-col p-6 lg:hidden">
          ...
          <nav className="flex flex-col gap-6 text-2xl font-semibold">
            {navLinks.map((item, i) => (
              <a key={item} href="#" className="text-white hover:text-[#C41A1A] transition-colors" onClick={() => {
                setMenuAberto(false)
                if (i === 0) router.push('/')
                if (i === 1) router.push('/oculos-de-sol')
              }}>
      ...
      <section className="max-w-7xl mx-auto px-4 lg:px-6 py-10 lg:py-14">
        <div className="border-b border-gray-300 pb-6 mb-10">
          <h1 className="text-3xl lg:text-4xl font-black tracking-widest text-black">ÓCULOS DE SOL</h1>
          <p className="text-gray-500 mt-2 text-sm tracking-wide">{produtos.length} modelos disponíveis</p>
        </div>
